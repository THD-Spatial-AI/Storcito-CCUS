...
const (
	resultStyleClassified   = "result_classified"
	resultStyleModeratePlus = "result_moderate_plus"
	resultStyleHighPlus     = "result_high_plus"
	resultStyleVeryLow      = "result_level_1"
	resultStyleLow          = "result_level_2"
	resultStyleModerate     = "result_level_3"
	resultStyleHigh         = "result_level_4"
	resultStyleVeryHigh     = "result_level_5"
)

type resultStyleDefinition struct {
	name          string
	title         string
	visibleValues []int
}

var resultStyleDefinitions = []resultStyleDefinition{
	{name: resultStyleClassified, title: "Result - contextual classified overlay", visibleValues: []int{1, 2, 3, 4, 5}},
	{name: resultStyleModeratePlus, title: "Result - moderate and above", visibleValues: []int{3, 4, 5}},
	{name: resultStyleHighPlus, title: "Result - high and above", visibleValues: []int{4, 5}},
	{name: resultStyleVeryLow, title: "Result - very low", visibleValues: []int{1}},
	{name: resultStyleLow, title: "Result - low", visibleValues: []int{2}},
	{name: resultStyleModerate, title: "Result - moderate", visibleValues: []int{3}},
	{name: resultStyleHigh, title: "Result - high", visibleValues: []int{4}},
	{name: resultStyleVeryHigh, title: "Result - very high", visibleValues: []int{5}},
}
...
type GeoServerService struct {
	...
	resultStylesMu    sync.Mutex
	resultStylesReady bool
}
...
func (s *GeoServerService) GetBoundsForResult(resultID uint) (map[string]interface{}, error) {
	...
	if err := s.ensureResultStylesReady(); err != nil {
		logger.ForComponent("geoserver").Warnf("failed to provision result styles while reading bounds result_id=%d err=%v", resultID, err)
	}
...
func (s *GeoServerService) CreateLayer(ctx context.Context, result *models.ModelResult) error {
	...
	if err := s.ensureResultStylesReady(); err != nil {
		log.Warnf("failed to provision result styles result_id=%d err=%v", result.ID, err)
	}
...
func (s *GeoServerService) applyStyleToLayer(layerName string) error {
	styleName := resultStyleClassified
	url := fmt.Sprintf("%s/rest/layers/%s:%s", s.baseURL, s.workspace, layerName)
...
func (s *GeoServerService) ensureResultStyles() error {
	var errs []string
	for _, def := range resultStyleDefinitions {
		if err := s.upsertSLDStyle(def.name, buildResultStyleSLD(def)); err != nil {
			errs = append(errs, fmt.Sprintf("%s: %v", def.name, err))
		}
	}
	if len(errs) > 0 {
		return fmt.Errorf("style provisioning failed: %s", strings.Join(errs, "; "))
	}
	return nil
}

func (s *GeoServerService) ensureResultStylesReady() error {
	s.resultStylesMu.Lock()
	defer s.resultStylesMu.Unlock()

	if s.resultStylesReady {
		return nil
	}
	if err := s.ensureResultStyles(); err != nil {
		return err
	}
	s.resultStylesReady = true
	return nil
}
...
type classColorMapEntry struct {
	quantity int
	label    string
	color    string
	opacity  float64
...
var dailyLayerKey = regexp.MustCompile(`^layer_(\d{4}-\d{2}-\d{2})$`)

// SampleDailyDistributions returns one class histogram per published layer_<date> layer.
func (s *GeoServerService) SampleDailyDistributions(ctx context.Context, resultID uint, sampleCount int) ([]DailyDistribution, error) {
	...
	for _, component := range components {
		key := strings.TrimSpace(component.Key)
		match := dailyLayerKey.FindStringSubmatch(key)
		if match == nil {
			continue
		}
	...
	for _, day := range days {
		...
		for i := 0; i < gridSize; i++ {
			for j := 0; j < gridSize; j++ {
				...
				distribution[levelFromScore(value)]++
				validSamples++
			}
		}
...
func levelFromScore(value float64) string {
